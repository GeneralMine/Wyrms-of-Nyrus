 ...
 * You're welcome. <3
 */
public abstract class EntityWyrmFlying extends EntityMob implements IAnimatable, IMob {
    ...
    protected void makeAllTargets() {
        ...
        if (Loader.isModLoaded("lycanitesmobs") && getAttackMobs()) {
            ...
            public boolean apply(EntityMob target) {return(target instanceof EntityLiving && !((target instanceof EntityCreeper) || (target instanceof EntityWyrmFlying)));}
        }));}
    }
    ...
    protected void afterMobs() {
        this.targetTasks.addTask(4, new EntityAINearestAttackableTarget<>(this, EntityMob.class, 2, true, false, new Predicate<EntityMob>() {
            public boolean apply(EntityMob target) {
                if (getWillAttackCreepers()) return !(target instanceof EntityWyrmFlying);
                else return !((target instanceof EntityCreeper) || (target instanceof EntityWyrmFlying));
            }
        }));
    }
